...
/**
 * Fetches every followed feed and, for each one that answers, replaces its
 * slot in [cache] — the source Home's topic rows read from. A feed that
 * fails to load — down, moved, not actually a feed — keeps whatever it last
 * had cached rather than going blank, and does not block the rest from
 * syncing. Returns how many feeds actually answered, for the caller to
 * report.
 */
suspend fun syncFeeds(client: HttpClient, feeds: List<Feed>, cache: FeedPostCache): Int {
    ...
    for (feed in feeds) {
        val entries = runCatching { fetchFeed(client, feed.url) }.getOrNull() ?: continue
