...
## Complexity
Traversal | O(V + E) | O(V) | Each vertex is visited once and each edge examined once, from each end for an undirected graph.
Recursive space | — | O(V) | Worst case the call stack holds every vertex — a path graph, or a long chain.
Cycle detection | O(V + E) | O(V) | Same traversal; the three-state colouring is what reads the answer off it.
Topological sort | O(V + E) | O(V) | Push each vertex when it *finishes*, then reverse the resulting order.
Path finding | O(V + E) | O(V) | Finds a path quickly. Says nothing about it being short.
...
## Steps
1. **Push the start** onto the stack — or make the first call, which does the same thing.
2. **Pop a node.** If it has already been visited, discard it and pop again; duplicates reach the stack legitimately.
3. **Mark it visited immediately**, before touching its neighbours.
4. **Push every unvisited neighbour.** Because a stack is last in, first out, the most recently pushed is explored next — that is the depth.
5. **Repeat until the stack empties.** Popping past a dead end is exactly the backtracking; nothing extra is needed to make it happen.
