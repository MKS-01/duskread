...
## Complexity
Access by index | O(n) | O(1) | No address arithmetic is possible — you have to walk.
Search | O(n) | O(1) | Same walk, comparing as you go.
Insert / delete at head | O(1) | O(1) | The head reference is always in hand.
Insert / delete given the node | O(1) | O(1) | Pure pointer rewiring, nothing shifts.
Insert / delete by value | O(n) | O(1) | Dominated by the search, not the write.
Storage | — | O(n) | Each node carries a pointer, so more memory per element than an array.
...
## Steps
1. To insert after a node you already hold: create the new node.
2. Point the new node's `next` at the current node's `next`.
3. Point the current node's `next` at the new node. Order matters — reverse these two and you lose the rest of the list.
4. To delete the node after one you hold: point its `next` past the victim, to `victim.next`.
5. In a garbage-collected language the orphaned node is now unreachable and will be collected; in C you would free it here.
