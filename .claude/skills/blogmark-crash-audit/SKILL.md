...
7. **Reproduce the specific race live, don't just patch and hope.** A rapid
   ...
   see `blogmark-verify` for the device-testing mechanics. Confirm the crash
...
Fix in the same pass, verify with `blogmark-verify`'s device loop, and if the
