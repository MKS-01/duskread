...
Black) and `MonoScheme` (Ink) — see `duskread-design-system`. A literal
...
triggering action, not one tap and a wait. See `duskread-crash-audit` for
when this matters.
...
# ... exercise it ...
adb logcat -d 2>/dev/null | grep -iE "FATAL EXCEPTION|duskread.*(Exception|Error)"
```
